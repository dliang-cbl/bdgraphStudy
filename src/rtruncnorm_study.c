...
double do_rtruncnorm_study(double a, double b, double mean, double sd){
  double ret;
  //double lwr,upr,limit;
  //lwr = fabs(mean - 10 * sd);
  //upr = fabs(mean + 10 * sd);
  //limit = ((lwr>upr)? lwr:upr);
  //Rprintf("limit=%f\n",limit);
  if (a > -DBL_MAX && b < DBL_MAX){
    //SAMPLER_DEBUG("[a,b]", a, b);
    ret = r_truncnorm(a,b,mean,sd);
  } else if (a < -DBL_MAX && b < DBL_MAX){
    //SAMPLER_DEBUG("(,b]", -9999, b);
    ret = r_righttruncnorm(b,mean,sd);
  } else if (a > -DBL_MAX && b > DBL_MAX){
    //SAMPLER_DEBUG("[a,)", a, 9999);
    ret = r_lefttruncnorm(a,mean,sd);
  } else if (a < -DBL_MAX && b > DBL_MAX){
    //SAMPLER_DEBUG("(,)", -9999, 9999);
    ret = rnorm(mean,sd);
  } else {
    ret = NA_REAL;
  }
