...
// DL Edit Use truncnorm package to update Z. 06-28-2018  
#include "copula.h"
// DL edit 06-28-2018
...
void get_mean( double Z[], double K[], double *mu_ij, double *sigma, int *i, int *j, int *n, int *p )
{
	...
	for( k = 0; k < col; k++ ) {
	  ...
	  //Rprintf("z=%f,k=%f,mu=%f\n",Z[k*number+row],K[col*dim+k],mu);
	}
...
void get_bounds( double Z[], int R[], double *lb, double *ub, int *i, int *j, int *n )
{
  ...
  // DL- edits 06-30-2018: change this to R macro
	//double low_b = -1e308, upper_b = +1e308;
	double low_b = R_NegInf, upper_b = R_PosInf;
...
void copula( double Z[], double K[], int R[], int *n, int *p )
{
	...
	GetRNGstate();
	// DL edit 06-26-2018 I don't understand that this step can be parallel-same below.
	//#pragma omp parallel
	//
	{	
		...
		{   
			...
			//DL-Edit 06-28-2018 use rtruncnorm to sample;
			...
			Z[ counter ] = do_rtruncnorm_study(lb,ub,mu_ij,sd_j);
			//Rprintf("i=%d,j=%d,mu=%f,sd=%f,lb=%f,ub=%f,z=%f\n",i+1,j+1,mu_ij,sd_j,lb,ub,Z[counter]);
		}
	}
	PutRNGstate();
}
