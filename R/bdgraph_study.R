...
## Modification  2020, Dong Liang
##
## modification to allow saving latent variables from successive
## reversible jump MCMC batches
##
## any error should not be attributed to the original author
##
bdgraph.study = function( data, n = NULL, method = "ggm", algorithm = "bdmcmc",
...
{
	check.os( os = 2 ,verbose=FALSE)	## Dong- Turn off message;
	if( cores == "all" ) cores = detect_cores()

	tmp   <- .C( "check_nthread", cores = as.integer(cores),PACKAGE = "bdgraphStudy")
	cores <- tmp $ cores

	.C( "omp_set_num_cores", as.integer( cores ),PACKAGE = "bdgraphStudy")
	...
	if( class( g.start )[1] == "bdgraph" )
	{
		G <- g.start $ last_graph
		K <- g.start $ last_K
		## DL 06-21-18
		if(method=="gcgm"){
		  ## DL Debug 06-30-18 the following clause is check
	    if(!any(is.nan(g.start $ last_Z))){
		    Z <- g.start $ last_Z
		    S <- t(Z) %*% Z
		    Ds <- D + S
		    Ts <- chol( solve(Ds))
	...
	if( class( g.start )[1] == "ssgraph" )
	{
	    G <- g.start $ last_graph
	    K <- g.start $ last_K
	}

	if( class( g.start )[1] == "sim" )
	{
		G <- as.matrix( g.start $ G )
		K <- as.matrix( g.start $ K )
	}

	if( class( g.start )[1] == "character" && g.start == "empty"  )
	{
		G = matrix( 0, p, p )
		K = G

		result = .C( "rgwish_c", as.integer(G), as.double(Ts), K = as.double(K), as.integer(b_star), as.integer(p),PACKAGE = "bdgraphStudy")
		K      = matrix ( result $ K, p, p )
	}

	if( class( g.start )[1] == "character" && g.start == "full" )
	{
		G         = matrix( 1, p, p )
		diag( G ) = 0
		K         = matrix( 0, p, p )

		result = .C( "rwish_c", as.double(Ts), K = as.double(K), as.integer(b_star), as.integer(p),PACKAGE = "bdgraphStudy")
		K      = matrix ( result $ K, p, p )
	}
	...
	#cat( mes, "\r" ) ## Dong : turn off iteration, move to helper script
	...
	if( save.all == TRUE )
	{
		...
		if( ( method == "ggm" ) && ( algorithm == "rjmcmc" ) )
		{
			result = .C( "ggm_rjmcmc_map", as.integer(iter), as.integer(burnin), G = as.integer(G), as.integer(g_space), as.double(g_prior), as.double(Ts), K = as.double(K), as.integer(p),
						all_graphs = as.integer(all_graphs), all_weights = as.double(all_weights), K_hat = as.double(K_hat),
						sample_graphs = as.character(sample_graphs), graph_weights = as.double(graph_weights), size_sample_g = as.integer(size_sample_g),
						as.integer(b), as.integer(b_star), as.double(Ds), as.integer(print),PACKAGE = "bdgraphStudy")
		}
		...
		if( ( method == "gcgm" ) && ( algorithm == "rjmcmc" ) )
		{
			result = .C( "gcgm_rjmcmc_map", as.integer(iter), as.integer(burnin), G = as.integer(G), as.integer(g_space), as.double(g_prior), as.double(Ts), K = as.double(K), as.integer(p),
						...
						all_graphs = as.integer(all_graphs), all_weights = as.double(all_weights), K_hat = as.double(K_hat),
						sample_graphs = as.character(sample_graphs), graph_weights = as.double(graph_weights), size_sample_g = as.integer(size_sample_g),
						as.integer(b), as.integer(b_star), as.double(D), as.double(Ds), as.integer(print),PACKAGE = "bdgraphStudy")
		}
...
## ------------------------------------------------------------------------------------------------|
	...
	if( save.all == TRUE )
	{
		...
		## DL 06-16-18: Save the last Z;
		last_Z <- matrix(NA,n,p)
		if(method=="gcgm"){
		  last_Z <- matrix(result[[9]],n,p)
		}
		## DL 06-21-18: Save the last Z;

		output = list( sample_graphs = sample_graphs, graph_weights = graph_weights, K_hat = K_hat, all_graphs=all_graphs,
					all_weights = all_weights, last_graph = last_graph, last_K = last_K, last_Z = last_Z , method=method)
	}else{
